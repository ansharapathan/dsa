...
node*create_post(char postfix[20]) 
{
    ...
    for (int i = 0; postfix[i] != '\0'; i++) 
        ...
        if (isalnum(token))         //isalnum to check whether a character is alphanumeric or not,
                                   //Characters such as punctuation marks, symbols, and whitespace 
                                   //are not considered alphanumeric.
            {
                p = new node();
                p->data = token;
                p->left = NULL;
    ...
    while (s.isempty() != 1) 
        {
            ...
            {
                s.push(t);
                s1.push((node *) 1);//assigning node 1
                t = t->right;
                while (t != NULL) 
                    {
                    s.push(t);
